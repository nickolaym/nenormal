...
- возвращающая успешный результат
  - пару `succes` (строка, rule_state_t)
  - либо признак неудачи - тип `fail`.
...
В коде, чтобы отличать эти строки, введён концепт `RuleInput`, который сейчас тождественнен `CtStr`.
...
Из тех же соображений признак продолжения/финиша (он повлияет на цепочку следующих вычислений)
возвращается как `ct<rule_state_t>`.

Таким образом, возвращаемый успех - это шаблон
```cpp
template<CtStr T, rule_state_t S> struct success { T text; S state; };
```

А итоговый тип результата - это концепт `RuleOutput<T> = Fail<T> || Success<T>`.
...
Поэтому структура `rules<Rule auto rs...>` также является представителем концепта Rule.

Также это значит, что подпрограммы сами можно группировать в ещё большие подпрограммы.

### Цикл исполнения

Цикл является функцией `rule_loop`, принимающей на вход строку и возвращающей `success`.
Он, очевидно, не может вернуть `fail` (разве что мы внедрим какие-то системы диагностики, например, защиту от зависания).
...
while (true) {
    ...
    if (failed(dst) || dst.state == final_state) return src;
    src = dst.text;
}
```
